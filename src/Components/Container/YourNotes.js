...
export class YourNotes extends Component {
    ...
    render() {
        ...
        let layout2 = this.props.notes.filter(
            note => (note.title + " " + note.textBody + " " + note.tags + " " + note.recipetime + " " + note.ingredients + " " + note.steps).toLowerCase().includes(`${this.props.currentSearch.sidebarSearch}`.toLowerCase())).map(
